...
const Home = () => {
    ...
    const indexPage = useSelector((state) => state.indexPage); //revisar
    ...
    const [showPokemons, setShowPokemons] = useState([]);//estoaaaa
    ...
    const handleChangeStorage = (event) => {
        dispatch(setStoragePokemons(event.target.value)); //se ejecuta cuando cambio la opcion de almacenamietno
    }
    const handleChangeType = (event) => {
        dispatch(setTypesPokemons(event.target.value));//cambio en la seleccion de tipos
    };
    const handleChangeOrder = (event) => {
        dispatch(setOrderPokemons(event.target.value)); //cambio en la opcion de orden 
    }
    useEffect(() => { //estooooooooo

        setShowPokemons(updatedShowPokemons);//actualializa los pokemones mostrados
