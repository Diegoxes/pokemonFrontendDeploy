...
const Form = ({ addPokemon, allTypes }) => { 
   ...
   const [errors, setErrors] = useState({});//almacena msj de error
   const [send, setSend] = useState(false); // habilita o deshabilita el boton de envio 

    useEffect(() => {     
        setErrors(validate(newPokemon));  //se ejecuta cada que newpokemon cambie
    }, [newPokemon]);
    ...
    const handleSubmit = (event) => { //se llama cuando envio el formulario 
        event.preventDefault(); //pa q no se actualize
        addPokemon(newPokemon); //agrega el nuevo pokemon
        setNewPokemon( //vacia el formulario
            {
                name: '',
                image: '',
                hp: '',
    ...
    const handleChange = (event) => {  // detecta los cambios en el formulario  y actualiza el estado
        const { name, value } = event.target;

        if (name !== 'types') {
            setNewPokemon({ ...newPokemon, [name]: value }); //name del input , e inserta el valor mas todo lo que hay en newPokemon
        } else {
    ...
    return (
        <div className={styles.container}>
            ...
            <form onSubmit={handleSubmit} autoComplete='off' >
                ...
                <div className={styles.input}>
                    <label className={styles.label}>  {/*TYPES*/}
                        Types:
